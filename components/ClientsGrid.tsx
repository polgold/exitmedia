      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {clients.map((client, i) => {
            const description = c.descriptions[client.domain];
            return (
              <MotionIn key={client.domain} delay={i * 0.04}>
                <a
                  ...
                  className="group flex h-full flex-col relative overflow-hidden rounded-2xl border border-border bg-surface"
                >
                  ...
                  <div className="flex flex-1 items-start justify-between gap-4 p-5">
                    <div className="min-w-0 flex-1">
                      ...
                      {description && (
                        <p className="mt-3 text-sm text-muted leading-snug text-pretty">
                          {description}
                        </p>
                      )}
                    </div>
                    <ArrowUpRight
                      size={18}
                      className="text-muted shrink-0 mt-1 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-accent"
                    />
                  </div>
                </a>
              </MotionIn>
